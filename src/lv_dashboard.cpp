...
void Create() {
  ...
  timer->setPeriod(1000).setCb(TimerCb).ready();
  ...
  btn->addStyle(
      btnStyle.get(),
      0);  // or btn->addStyle(btnStyle, 0);  if you are using raw pointers
  ...
  timeline->add(0, zoomin.get()).add(1100, getout.get()).start();
